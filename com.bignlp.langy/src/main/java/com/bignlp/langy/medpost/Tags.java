...
/*
 * History: 2.0, June 9, 2005 - Cleaned up MAX_WORDS to be sent in at creation
 * 
 * 1.0, August 18, 2004 - Epoch
 */

public class Tags {
	public static final int num_tags = 60;
	private static final String tag_str[] = { "''", "(", ")", ",", ".", ":",
			"CC", "CS", "CSN", "CST", "DB", "DD", "EX", "GE", "II", "JJ",
			"JJR", "JJT", "MC", "NN", "NNP", "NNS", "PN", "PND", "PNG", "PNR",
			"RR", "RRR", "RRT", "SYM", "TO", "VBB", "VBD", "VBG", "VBI", "VBN",
			"VBZ", "VDB", "VDD", "VDG", "VDI", "VDN", "VDZ", "VHB", "VHD",
			"VHG", "VHI", "VHN", "VHZ", "VM", "VVB", "VVD", "VVG", "VVGJ",
			"VVGN", "VVI", "VVN", "VVNJ", "VVZ", "``" };
	 ...
	 * 
	 * @param pos
	 *            Position within tag_str requested
	 ...
	 * 
	 * @param tag
	 *            Tag to look for within tag_str
	 ...
	 * 
	 * @param pos1
	 *            Position one within dw requested
	 * @param pos2
	 *            Position two within dw requested
	 ...
	 *            Position two within dw requested
	 * @param val
	 *            Value to be inserted at pos1, pos2
	 **/
	 ...
	 * @param lex_backoff
	 *            Array of Lex backoff values for each tag
	 * @param count
	 *            Array of Lexical occurrences counts
	 **/
	...
	public void normalize(int numWords, double lex_backoff[], double count[]) {
		...
		for (int i = 0; i < numWords; i++) {
			...
			if (sum == 0.0) {
				System.out.println("Cannot continue because word " + i
						+ " has no possible tags.");
				System.exit(999);
			} // fi
		} // for numWords
	} // normalize
	 ...
	 * 
	 * @param wordPos
	 *            Position of this word in the dw array
	 * @param tagPos
	 *            Which tag we are affecting the value for
	 * @param cond
	 *            Ensuring (true) tag or Removing (false) tag probability
	 **/
	...
	public void checkTags(String word, int i, int wordLen) {
		if (isAllDigit(word))
			...
			if ((wordLen == 1) && !Character.isLetterOrDigit(word.charAt(0))) {
			...
			for (int j = 0; j < wordLen; j++) {
				...
				if (!b) {
					if (Character.isDigit(word.charAt(j)))
						b = true;
					else if ((j > 0) && Character.isUpperCase(word.charAt(j)))
						b = true;
				} // fi
			} // for

			if (a && b) {
				if ((wordLen > 0) && (word.charAt(wordLen - 1) == 's'))
					tagSet(i, NNS, true);
				else
					tagSet(i, NN, true);
			} // fi
