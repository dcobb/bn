...
public class MetaMapApiImpl implements MetaMapApi {
	...
	/**
	 * Instantiate api using MetaMap server specified by hostname with specified
	 * timeout.
	 * 
	 * @param serverHostname
	 *            hostname of metamap server
	...
	public String getOptions() {
		...
		try {
			...
			QueryAnswer answer = session
					.executeQuery("get_options(AllOptions)");
	...
	public void invokeSetOptions(String optionListString) {
		try {
			...
			Bindings bindings = new Bindings()
					.bind("Options", optionListString);
			QueryAnswer answer = session.executeQuery("set_options(Options)",
					bindings);
			...
			throw new RuntimeException(
					e.getMessage()
							+ ": Check to see if mmserver is running, or if port and hostname specified for the mmserver are correct.",
					e);
		} catch (Exception e) {
			System.err.println("Error when querying Prolog Server: "
					+ e.getMessage() + '\n');
	...
	public void unsetOptions(String optionString) {
		try {
			...
			QueryAnswer answer = session.executeQuery("unset_options(Options)",
					bindings);
		} catch (ConnectException e) {
			System.err.println("Error when querying Prolog Server: "
					+ e.getMessage() + '\n');
	...
	public void unsetOptions(List<String> options) {
		try {
			...
			StringBuffer sb = new StringBuffer();
			sb.append("[");
			for (Iterator<String> optIter = options.iterator(); optIter
					.hasNext();) {
				sb.append("'").append(optIter.next()).append("'");
				if (optIter.hasNext())
					sb.append(",");
	...
	public void resetOptions() {
		try {
			...
			QueryAnswer answer = session.executeQuery("reset_options");
		} catch (ConnectException e) {
			System.err.println("Error when querying Prolog Server: "
					+ e.getMessage() + '\n');
			throw new RuntimeException(
	...
	public List<Result> processCitationsFromString(String aString) {
		...
		/*
		 * split string into multiple citations for content separated by blank
		 * lines.
		 */
		...
		for (int i = 0; i < citations.length; i++) {
			...
			try {
				...
				QueryAnswer answer = session.executeQuery(
						"process_string(E,Output)", bindings);
