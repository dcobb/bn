...
/**
 * An implemention of the <code>Result</code> interface, a container for the
 * result of a MetaMap query.
 * <p>
...
public class ResultImpl implements Result {
	 ...
	 * 
	 * @param mmoTerm
	 *            a prolog term containing MetaMap machine output.
	 */
	public ResultImpl(PBTerm mmoTerm) throws Exception {
		if (mmoTerm.isListCell()) {
			...
			throw new Exception(
					"resulting term is not a legal machine output termlist");
		}
	}
	 ...
	 *            a prolog term containing MetaMap machine output.
	 * @param theInputText
	 *            the input text that produced this result.
	 */
	...
	/**
	 * @return a <code>String</code> containing the result as MetaMap machine
	 *         output.
	 */
	...
	public final List<AcronymsAbbrevs> getAcronymsAbbrevs() throws Exception {
		...
		PBTerm listTerm = TermUtils.getListElement(mmoTermList,
	 ...
	 * @return a <code>List</code> of
	 */
	public final List<AcronymsAbbrevs> getAcronymsAbbrevsList()
			throws Exception {
		List<AcronymsAbbrevs> aasList = new ArrayList<AcronymsAbbrevs>();
		PBTerm listTerm = TermUtils.getListElement(mmoTermList,
				ACRONYMS_ABBREVS_INDEX);
	...
	public List<Utterance> getUtteranceList() throws Exception {
		...
		for (int i = FIRST_UTTERANCE_INDEX; i <= this.mmoTermList.length(); i++) {
			if (TermUtils.getListElement(this.mmoTermList, i).getName()
					.equals("utterance")) {
				utteranceList.add(new UtteranceImpl(TermUtils.getListElement(
						this.mmoTermList, i), i, this));
			}
		}
	 ...
	 * starting at start in machine output. It is recommended to use the
	 * <code>Utterance</code> implementation of getPCMList rather than this one.
	 * 
	 * @param utterancePosition
	 *            position of relevant utterance
	...
	private List<PCM> getPCMList(int utterancePosition) throws Exception {
		...
		for (int i = start; i <= this.mmoTermList.length(); i = i + 3) {
			if (TermUtils.getListElement(this.mmoTermList, i).getName()
				...
				int j = i + 1;
				if (TermUtils.getListElement(this.mmoTermList, j).getName()
						.equals("candidates")) {
					candidates = TermUtils.getListElement(this.mmoTermList, j);
				} else {
					TermUtils.getListElement(this.mmoTermList, j).getName()
							.equals("utterance");
					break;
				}
				j++;
				...
				TermUtils.getListElement(this.mmoTermList, i).getName()
						.equals("utterance");
				break;
			}
		}
	...
	class AcronymsAbbrevsImpl implements AcronymsAbbrevs {
		...
		public String getAcronym() {
			return aasTerm.getArgument(1).getArgument(1).getArgument(1)
					.getString();
		}

		public String getExpansion() {
			return aasTerm.getArgument(1).getArgument(1).getArgument(2)
					.getString();
		};
		...
		public List<String> getCUIList() {
			...
			/*
			 * get the second argument which contains the prolog list containing
			 * the cuis
			 */
	...
	class NegationImpl implements Negation {
		...
		public List<ConceptPair> getConceptPairList() throws Exception {
			...
			for (int i = 1; i <= plist.length(); i++) {
				...
				pairList.add(new ConceptPairImpl(
						term.getArgument(1).toString(), term.getArgument(2)
								.toString()));
				pTerm = pTerm.tail();
			}
			return pairList;
	...
	class UtteranceImpl implements Utterance {
		...
		/**
		 * @param newUtteranceTerm
		 *            PrologBeans term representation of utterance term.
		 * @param listPosition
		 *            position of term in machine output term list.
		 * @param theResult
		 *            the MetaMap result.
		 */
		public UtteranceImpl(PBTerm newUtteranceTerm, int listPosition,
				ResultImpl theResult) throws Exception {
		...
		public String toString() {
			try {
				return "utterance: " + this.getId() + "," + this.getString()
						+ "," + this.getPosition();
			} catch (Exception e) {
