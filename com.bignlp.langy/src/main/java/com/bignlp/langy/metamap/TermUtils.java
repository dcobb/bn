...
public class TermUtils {
	...
	public static List<String> getAtomStringListArgument(PBTerm parentTerm,
			int argnum) {
	...
	public static List<String> getStringListArgument(PBTerm parentTerm,
			int argnum) {
		List<String> stringList = new ArrayList<String>();
		PBTerm term = parentTerm.getArgument(argnum);
	...
	public static List<Integer> getIntegerListArgument(PBTerm parentTerm,
			int argnum) {
		List<Integer> integerList = new ArrayList<Integer>();
		PBTerm prologList = (PBTerm) parentTerm.getArgument(argnum);
		for (int i = 1; i <= prologList.length(); i++) {
			integerList.add(Integer.parseInt(getListElement(prologList, i)
					.toString()));
		}
		return integerList;
	}
	...
	public static Position getPositionArgument(PBTerm parentTerm, int argnum)
		...
		PBTerm term = parentTerm.getArgument(argnum);
		if (term.isCompound())
			return new PositionImpl(term.getArgument(1).intValue(), term
					.getArgument(2).intValue());
		else
			throw new Exception("supplied term is not a compound term.");
	}

	public static List<Position> getPositionListArgument(PBTerm parentTerm,
			int argnum) {
	...
	public static List<Object> getMatchMapTree(PBTerm prologList)
			throws Exception {
		...
		for (int i = 1; i <= prologList.length(); i++) {
			if (getListElement(prologList, i).isListCell()) {
				...
				objectList.add(new Integer((int) getListElement(prologList, i)
						.intValue()));
			}
		}
		return objectList;
