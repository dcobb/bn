...
public class PCMBase implements PCM {
	 ...
	 * 
	 */
	public PCMBase(PBTerm aPhraseTerm, PBTerm aCandidatesTerm,
	 ...
	 * .
	 * 
	 * @return a Candidates instances containing a List of candidate Ev
	 *         instances.
	 */
	...
	public final List<Ev> getCandidateList() throws Exception {
		 ...
		 * output, the <pre> candidates(<ListOfCandidates>) </pre> term now
		 * includes the counts of total, excluded, pruned, and remaining
		 * candidates: i.e., <pre> candidates(TotalCandidatesCount,
		 * ExcludedCandidateCount, PrunedCandidateCount, RemainingCandidates,
		 * <ListOfCandidates>) </pre>
		 */
		PBTerm prologList = this.candidatesTerm.getArgument(5);
		PBTerm term = prologList;
		for (int i = 1; i <= prologList.length(); i++) {
	...
	class MatchMapImpl implements MatchMap {
		// The match map is represented in prolog result as a set of nested
		// lists:
		// organization:
		// [[[phrase-match-start,
		// phrase-match-end],[concept-match-start,concept-match-end],
		// lexical-variation]]
		// Example 1.: This mapping shows word 1 of the phrase maps to word 1 of
		// the concept with 0 lexical variation
		//
		// [[[1,1],[1,1],0]]
		...
		//
		//
		// Example 2.: This shows word 2 of the phrase maps to word 1 of the
		// concept with 0 lexical variation and word 3
		...
		public int getLexMatchVariation() {
			if (this.prologList.length() > 2) {
				if (TermUtils.getListElement(prologList, 3).isInteger())
					return (int) TermUtils.getListElement(prologList, 3)
							.intValue();
				else
					return 0;
			} else {
	...
	class EvImpl implements Ev {
		...
		public String toString() {
			try {
				return this.getScore() + "," + this.getConceptId() + ","
						+ this.getConceptName() + "," + this.getPreferredName()
						+ ","
						+ this.getMatchedWords()
						+ ","
						+ this.getSemanticTypes()
						// + this.getMatchMap() + ","
						+ ", isHead: " + this.isHead() + ", isOverMatch: "
						+ this.isOvermatch() + "," + this.getSources() + ","
						+ this.getPositionalInfo() + ", pruning status: "
						+ this.getPruningStatus() + ".";
			} catch (Exception e) {
	...
	class CandidatesImpl implements Candidates {
		...
		public int getTotalCandidatesCount() {
			return (int) this.candidatesTerm.getArgument(1).intValue();
		}

		public int getExcludedCandidateCount() {
			return (int) this.candidatesTerm.getArgument(2).intValue();
		}

		public int getPrunedCandidateCount() {
			return (int) this.candidatesTerm.getArgument(3).intValue();
		}

		public int getRemainingCandidates() {
			return (int) this.candidatesTerm.getArgument(4).intValue();
		}
